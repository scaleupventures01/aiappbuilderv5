/**
 * DataAnalystAgent - FULLY FUNCTIONAL agent generated from data-analyst.md
 * Use this agent for SQL/reporting, dashboards, and business insights.
 * 
...
export class DataAnalystAgent extends Agent {
  constructor() {
    super({
      ...
      allowedTools: ["*"],
      metadata: {
        source: 'data-analyst.md',
        kpis: "KPI freshness/accuracy, dashboard adoption, decision latency.",
        collaborators: []
      }
    });
  ...
  async analyzeTask(task) {
    ...
    // Determine approach
    analysis.approach = 'Apply data-analyst expertise to solve the task';
  ...
  async performWork(task, analysis) {
    ...
    // Default work implementation for data-analyst
  ...
  canHandleTask(task) {
    ...
    if (task.requirements?.role === 'data-analyst') return true;
    ...
    const desc = task.description?.toLowerCase() || '';
    desc.includes('query') || desc.includes('build') || desc.includes('interpret') || desc.includes('provide') ? true : false;
...
export const data_analystAgent = new DataAnalystAgent();
