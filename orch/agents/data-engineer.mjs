/**
 * DataEngineerAgent - FULLY FUNCTIONAL agent generated from data-engineer.md
 * Use this agent for ETL pipelines, data warehousing, and data quality.
 * 
...
export class DataEngineerAgent extends Agent {
  constructor() {
    super({
      ...
      allowedTools: ["*"],
      metadata: {
        source: 'data-engineer.md',
        kpis: "Data freshness, data quality pass rate, pipeline reliability.",
        collaborators: []
      }
    });
  ...
  async analyzeTask(task) {
    ...
    // Determine approach
    analysis.approach = 'Apply data-engineer expertise to solve the task';
  ...
  async performWork(task, analysis) {
    ...
    // Default work implementation for data-engineer
  ...
  canHandleTask(task) {
    ...
    if (task.requirements?.role === 'data-engineer') return true;
    ...
    const desc = task.description?.toLowerCase() || '';
    desc.includes('etl') || desc.includes('pipeline') || desc.includes('data') || desc.includes('warehouse') || desc.includes('spark') ? true : false;
  ...
  getCapabilities() {
    return {
      ...
      specializations: ["spark","airflow","data-lakes"],
      tools: ["spark","kafka","airflow","dbt"]
    };
  }
}

// Create singleton instance
export const data_engineerAgent = new DataEngineerAgent();
